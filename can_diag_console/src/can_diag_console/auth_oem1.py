...
from .protocols import fmt_hex, parse_hex_bytes
...
class KwpAuthOem1Runner:
    """Runs the OEM1 KWP L3 auth sequence with user-provided key material."""
    ...
    def run(self) -> AuthRunResult:
        ...
        while state != AuthState.DONE:
            ...
            if state == AuthState.WAIT_USER_KEY:
                ...
                prompt = (
                    "Enter auth key payload as hex bytes: "
                )
                try:
                ...
                release_payload = (
                    bytes([0x31, 0x08])
                    + self._cfg.auth_key_length.to_bytes(4, "big")
                    + key_payload
                )
                ...
                for attempt in range(1, self._cfg.release_retries + 1):
                    self._emit(
                        f"[kwp-auth-sk] submit-key attempt {attempt}/{self._cfg.release_retries}"
                    )
                    ...
                    if release_resp is None:
                        ...
                        self._emit(
                            f"[kwp-auth-sk] submit-key negative -> {fmt_hex(release_resp)}"
                        )
                    else:
